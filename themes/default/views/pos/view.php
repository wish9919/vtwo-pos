        ...
        ?><!doctype html>
                ...
                <head>
                    ...
                    <style type="text/css" media="all">
                        body { color: #000; }
                        #wrapper { max-width: 700px; margin: 0 auto; padding-top: 20px; }
                ...
                ?>
                <div id="wrapper">
                    <div id="receiptData" style="width: auto; max-width: 700px; min-width:300px; margin: 0 auto;">
                        <div class="no-print">
                            <?php if ($message) { ?>
                        ...
                        <div id="receipt-data">
                            ...
                            <div style="text-align:center;">
                                    ...
                                    if ($store) {
                                        ...
                                        echo '<h1 style="font-family: Georgia !important;">' . $store->name . '</h1>';
                                        echo '<p style="text-align:center;">';
                                        echo '<h4>General Hardware Merchants</h4>';
                                        echo $store->address1;
                                        echo ', ' . $store->city . '<br> Tel: 0760147843 / 0764766969';
                                        echo '</p>';
                                ...
                                <div class="row justify-content-between">
                                    <div class="col-md-6"> 
                                <p>
                                     <?= '<strong>' . lang('date') . '</strong>' . ': ' . $this->tec->hrld($inv->date); ?> <br>
                                    <?= '<strong>' . 'Invoice No' . '</strong>'  . ': ' . $inv->id; ?><br>
                                    <?= '<strong>' . lang('sales_person') .  '</strong>'  . ': ' . $created_by->first_name . ' ' . $created_by->last_name; ?> <br>
                                    <br>
                                </p></div>
                                    <div class="col-md-6">
                                    <p style="text-align: right">
                                    <?=  '<strong>' . lang('customer') . '</strong>'  . ':  ' . $inv->customer_name; ?> <br> 
                                    <?php 
                                    if ($payments){
                                        foreach ($payments as $payment) {
                                            echo '<strong>Paid by</strong>' . ':  ' . '<span style="text-transform: uppercase;">' . $payment->paid_by .  '</span>' ;
                                        }
                                    }
                                    ?>
                                ...
                                <table class="table table-striped table-condensed">
                                    <thead>
                                        <tr>
                                            <th class="" style="width: 10%; border-bottom: 2px solid #ddd; text-align: left;">No</th>
                                            <th class="" style="width: 50%; border-bottom: 2px solid #ddd; text-align: left;"><?=lang('description');?></th>
                                            <th class="text-center" style="width: 12%; border-bottom: 2px solid #ddd;"><?=lang('quantity');?></th>
                                            <th class="text-center" style="width: 12%; border-bottom: 2px solid #ddd; text-align: center;"><?=lang('discount');?></th>
                                            <th class="text-center" style="width: 24%; border-bottom: 2px solid #ddd; text-align: right;"><?=lang('price');?></th>
                                    ...
                                    <tbody class="bill-table">
                                        ...
                                        foreach ($rows as $key=>$row) {
                                            $index = $key +1;
                                            $i = str_pad($index, 2, '0', STR_PAD_LEFT);
                                          echo '<tr class="sale-item-no">';
                                          echo '<td>' .  $i . '</td>';
                                            echo '<td>';
                                            ...
                                            echo $this->tec->formatMoney($row->real_unit_price + ($row->item_tax / $row->quantity)) . '</td><td class="text-right">' . $this->tec->formatMoney($row->subtotal) . '</td></tr>';
                                        }
                                        ?>
                                    </tbody>
                                    ...
                                    <tfoot style="margin-top: 20px">
                                        ...
                                        <tr>
                                            <th style="padding-top: 30px" class="text-left" colspan="3"><?= lang('total'); ?></th>
                                            <th style="padding-top: 30px" colspan="3" class="text-right"><?= $this->tec->formatMoney($inv->total + $inv->product_tax); ?></th>
                                        </tr>
                                        <?php
                                        if ($inv->order_tax != 0) {
                                            echo '<tr><th class="text-left" colspan="3">' . lang('order_tax') . '</th><th colspan="2" class="text-right">' . $this->tec->formatMoney($inv->order_tax) . '</th></tr>';
                                        }
                                        ...
                                        if ($inv->total_tax != 0) {
                                            echo '<tr><th class="text-left" colspan="3">' . lang('tax') . '</th><th colspan="2" class="text-right">' . $this->tec->formatMoney($inv->total_tax) . '</th></tr>';
                                        }
                                        ...
                                        if ($Settings->rounding) {
                                            ...
                                            <tr>
                                                <th class="text-left" colspan="3"><?= lang('rounding'); ?></th>
                                ...
                                if ($payments) {
                                    ...
                                    foreach ($payments as $payment) {
                                        echo '<tr style="padding: 20px !important;">';
